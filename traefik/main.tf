...
# DNS zone managed by AWS DNS.
data "aws_dns_managed_zone" "default" {
  name = "${var.dns_zone_name}"
}

# Root A record.
resource "aws_dns_record_set" "a_root" {
  name         = "${var.domain_name}."
  managed_zone = "${data.aws_dns_managed_zone.default.name}"
  type         = "A"
  ttl          = 300
...
# Wildcard A record.
resource "aws_dns_record_set" "a_wildcard" {
  name         = "*.${var.domain_name}."
  managed_zone = "${data.aws_dns_managed_zone.default.name}"
  type         = "A"
  ttl          = 300
